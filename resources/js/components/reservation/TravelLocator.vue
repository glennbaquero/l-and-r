	...
	<div class="bg-white lg:px-8 m-auto px-4 py-16 rounded-lg sm:px-6 sm:w-3/5 w-full z-10">
		...
		<div class="gap-2 grid grid-cols-1 show-selection-for-mobile sm:gap-10 sm:grid-cols-2 mt-4">
			...
			<div class="col-span-1 sm:col-span-2">
				<input ref="datepickerDepartureMobile" type="text" class="border-gray-400 w-full selection-3" v-model="data.departure_date" @change="travelDateChange" placeholder="Select Date" readonly>
			</div>
			...
			<!-- <div class="col-span-full sm:col-span-full">
				<button type="submit" class="bg-red-500 border border-transparent duration-150 ease-in-out focus:border-red-300 focus:outline-none focus:shadow-outline-red font-medium h-full inline-flex items-center justify-center px-4 py-2 sm:leading-5 sm:text-sm text-white transition w-full">
				    Search
				</button>
			</div> -->
		...
		<div class="grid grid-cols-4 show-selection-for-web mt-4">
			...
			<div class="col-span-2 sm:col-span-2">
				<input ref="datepickerDeparture" type="text" class="border-gray-400 w-full selection-3" v-model="data.departure_date" placeholder="Select Date" @change="travelDateChange" readonly>
			</div>
			<!-- <div class="col-span-2 sm:col-span-1">
	...
	export default {
		...
		watch: {
			...
			'data.departure_id'(val) {
				let city = _.find(this.cities, function(city) {
				  	return city.id == val;
				});

				this.$parent.payloads.departure_city = city;
			},
			...
			'data.arrival_id'(val) {
				...
				this.$parent.payloads.arrival_city = city;
			}
		...
		methods: {
			...
			init() {
				...
				axios.get(this.getCitiesUrl, { params: my_location } )
					.then(response => {
						...
						if(!_.isEmpty(this.$parent.$parent.savedPayload)) {
							this.data = this.$parent.$parent.savedPayload;
							this.selectedChanged()
							this.travelDateChange()
							setTimeout(() => {
								this.$parent.selectedDate(this.data.selected_date);
							}, 1000)
						} else {
			...
			setupFlatpickr() {
				...
				let $this = this;
				let default_date = !_.isEmpty(this.data.departure_date) ? this.data.departure_date.split(",") : [];

				setTimeout(() => {
					default_date.unshift(new Date());
			...
			passengerCount(type) {
				...
				this.$parent.$parent.trip_seat_selection_data.passenger_count = this.data.passenger_count;
			},
			...
			selectedChanged() {
				...
				axios.post(this.findAvailableTripUrl, this.data)
					.then(response => {
						...
						this.$parent.available_dates = response.data.available_dates;
						this.$parent.trips = response.data.trips;
						this.$parent.price = response.data.price;

						this.$parent.$parent.loading = false;
						// this.$parent.renderDate();
			...
			travelDateChange() {
				let departure_date = this.data.departure_date.split(",");

				if(!_.isEmpty(this.data.departure_date)) {
					this.$parent.selectedDate(departure_date[0])
				}
			}
		}
